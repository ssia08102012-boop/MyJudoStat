...
Поточна версія зберігається в `src/version.ts` як `VERSION`.

**Правило: кожен мердж у main = версія +1 (останній сегмент).**
Приклади: `v1.02.01` → `v1.02.02` → `v1.02.03`

При кожному коміті перед пушем — оновити `src/version.ts`.
